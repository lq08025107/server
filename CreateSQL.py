...
class SQLCluster():
    def __init__(self):
        self.ms = MSSQL()#getsqlcluster

    def insertOriginalInfo(self, nodeList):
        sql = "INSERT INTO DeepGlint(AlarmLevel, CutboardBox, CutboardTimeOffset, EventType, EventTypeProbability, FrameRate, HotspotId, Id, PeopleId, PlanetId, SceneId, SensorId, SliceLength, StartTime, TimeLength, UserId) VALUES("\
        + str(nodeList['AlarmLevel']) +","+ "'"+str(nodeList['CutboardBox'])+"'"+"," + "'" + str(nodeList['CutboardTimeOffset']) + "'"+ "," + str(nodeList['EventType'])+"," + str(nodeList['EventTypeProbability']) \
        +"," +"'"+ str(nodeList['FrameRate'])+"'"+"," + "'" +str(nodeList['HotspotId'])+"'" + "," + "'" +str(nodeList['Id'])+"'" +"," + "'" +str(nodeList['PeopleId'])+"'"\
        "," + "'" +str(nodeList['PlanetId'])+"'"+"," + "'" +str(nodeList['SceneId'])+"'"+"," + "'" +str(nodeList['SensorId'])+"'"+"," + "'" +str(nodeList['SliceLength'])+"'" \
        + "," + "'" + str(nodeList['StartTime']) + "'"+"," + "'" +str(nodeList['TimeLength'])+"'"+"," + "'" +str(nodeList['UserId'])+"'"+")"
    ...
    #根据设备name查询设备id
    def deviceName2deviceID(self, name):
        sql = "SELECT * FROM Device WHERE DeviceName = " + "'" +str(name) + "'"
        deviceid = self.ms.ExecQuery(sql)[0]['ID']
        return deviceid

    #根据报警类型查询报警类型id
    def alarmCode2alarmType(self, alarmCode):
        sql = "SELECT * FROM DicAlarmType WHERE AlarmType = " + str(alarmCode)
        alarmType = self.ms.ExecQuery(sql)[0]['ID']
        return alarmType
    ...
    def updatePackageFinishInfo(self, packageId, userName, time, record, stop):
        if userName == None:
            sql = "UPDATE AlarmPackage SET IsFinish =" + str(1) + ", FinishReason = " + "'" +str(stop) + "'"\
    ...
    def selectNameByAlarmType(self, id):

        sql = "SELECT * FROM DicAlarmType WHERE ID = " + str(id)
        name = self.ms.ExecQuery(sql)[0]['Name']

        return name

    def selectNameByAlarmLevel(self, id):
        sql = "SELECT * FROM DicAlarmLevel WHERE ID = " + str(id)
        name = self.ms.ExecQuery(sql)[0]['Name']
        return name
if __name__ == '__main__':
    sqlcluster = SQLCluster()
    print sqlcluster.selectNameByAlarmType(6)
